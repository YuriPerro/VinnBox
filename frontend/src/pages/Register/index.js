...
const Cadastro = () => {
  ...
  async function submitForm(event) {
    ...
    if (String(nome).length <= 2) alert("Nome curto demais!");
    else {
      var reg = /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
  ...
  function usersExists(email) {
    return new Promise(function (result, reject) {
      ...
      if (usersData) {
        ...
        if (userDB.length > 0) result(true);
        else result(false);
      } else result(false);
    });
  }
      ...
      <div className="card">
        ...
        <div className="body">
          <form onSubmit={submitForm} className="form-group">
            ...
            <input type="password" name="confirmation" id="conf" placeholder="Digite sua senha" />

            <span className="bottom-text">
              Ao se cadastrar, você confirma que leu e aceitou nossos&nbsp; Termos de Serviço e
