...
const Login = () => {
  ...
  async function recPassword() {
    ...
    var reg = /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/
    if (!reg.test(recEmail))
      alert("Email inválido!")
    else
      await usersExists(recEmail, "", true);
  }
        ...
        {!showPass ?
          <>
            <input
              ...
              id="recEmail"
            />
            <BaseButton onClick={() => recPassword()} type={"button"}> Recuperar senha </BaseButton>
          </>
          :
          <>
            <a
            ...
            <BaseButton onClick={() => closeModalRec()} type={"submit"}> Confirmar </BaseButton>
          </>
        }
      </BaseModal>
      <img
      ...
      <div className="card">
        ...
        <div className="body">
          <form onSubmit={submitForm} className="form-group">
            ...
            <a className="forgot-pass-link" onClick={() => setOpenModal(true)}>
              Esqueceu a senha ?
            </a>
